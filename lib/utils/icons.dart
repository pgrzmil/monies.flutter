...
class IconsHelper {
  ...
  static IconData tryParse(String value) {
    value = value.toLowerCase();

    IconData iconData;
    final code = int.tryParse(value);
    if (code != null) {
      iconData = iconFromCode(code);
    } else {
      iconData = IconsHelper.iconFromName(value);
    }

    return iconData;
  }

  static IconData iconFromName(String iconName, [IconData fallbackIcon]) {
    iconName = iconName.toLowerCase();

    if (_iconsMap.containsKey(iconName)) {
      final codePoint = _iconsMap[iconName];
      return IconData(codePoint, fontFamily: 'MaterialIcons');
    }
    return fallbackIcon;
  }

  static IconData iconFromCode(int iconCode, [IconData fallbackIcon]) {
    if (_iconsMap.containsValue(iconCode)) {
      return IconData(iconCode, fontFamily: 'MaterialIcons');
    }
    return fallbackIcon;
  }

  static int iconCodeFromName(String iconName) {
    iconName = iconName.toLowerCase();

    final code = int.tryParse(iconName);
    if (code != null) {
      return code;
    } else if (_iconsMap.containsKey(iconName)) {
      return _iconsMap[iconName];
    }
    return null;
  }

  static String iconNameFromCode(int codePoint) {
    if (_iconsMap.containsValue(codePoint)) {
      return _iconsMap.keys.firstWhere((x) => _iconsMap[x] == codePoint, orElse: () => "").toLowerCase();
    }
    return "";
  }

  static final _iconsMap = {
    "icons 3d_rotation": 0xe84d,
    "ac_unit": 0xeb3b,
    "access_alarm": 0xe190,
